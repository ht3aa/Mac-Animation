...
function getChildrenXCenterValues() {
  for (let i = 0; i < childernOfparentDiv.length; i++) {
    let childernXValue = Math.floor(
      childernOfparentDiv[i].getBoundingClientRect().x
    );
    let childernWidth = Math.floor(
      childernOfparentDiv[i].getBoundingClientRect().width
    );
...
parentDiv.addEventListener("mousemove", function (e) {
  ...
  for (let i = 0; i < childernOfparentDiv.length; i++) {
    let y =
      amplifier *
      Math.sqrt(range - Math.pow(xMouse - childrenXCenterValues[i], 2));
